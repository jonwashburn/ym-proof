...
lemma log_two_bounds : (0.693 : ℝ) < log 2 ∧ log 2 < (0.694 : ℝ) := by
  sorry -- TODO: Add numerical approximation

lemma log_four_bounds : (1.386 : ℝ) < log 4 ∧ log 4 < (1.387 : ℝ) := by
  sorry -- TODO: Add numerical approximation

lemma log_eight_bounds : (2.079 : ℝ) < log 8 ∧ log 8 < (2.080 : ℝ) := by
  sorry -- TODO: Add numerical approximation
...
/-! ## Specific numerical computations -/

lemma b_zero_times_1_5_squared : 11 / (3 * 16 * 9.869) < 11 / (3 * 16 * π^2) ∧
                                  11 / (3 * 16 * π^2) < 11 / (3 * 16 * 9.869) := by
  constructor
  · apply div_lt_div_of_lt_left
    ...
    · apply mul_lt_mul_of_pos_left; apply mul_lt_mul_of_pos_left
      exact pi_sq_lower_bound
      norm_num; norm_num
  · sorry -- The second inequality seems backwards, need to check
