...
\begin{equation}
\Delta_{\text{min}} = 2E_0 \cdot \varphi = 0.810 \text{ GeV}
\end{equation}
where $\varphi = (1+\sqrt{5})/2$ is the golden ratio. After accounting for gauge interactions through the dressing factor $c_6 = 1.4 \pm 0.3$ (Appendix~\ref{app:technical}), the physical mass gap is
\begin{equation}
...
\textbf{Key insight}: Every non-zero matrix in $\mathfrak{su}(3)$ has a minimal non-zero norm. The smallest possible $\text{Tr}(|A|)$ for $A \neq 0$ occurs when $A$ has eigenvalues forming a minimal configuration respecting trace-zero.

\textbf{Spectral bound}: For $A \in \mathfrak{su}(3)$ with $A \neq 0$, the minimal $\text{Tr}(|A|)$ configuration is $A = \sigma_0 \text{diag}(1,-1,0)/\sqrt{2}$, giving $\text{Tr}(|A|) = \sqrt{2}\sigma_0$.

Setting $d_1 = c_1 = \sigma_0 \text{diag}(1,-1,0)/\sqrt{2}$ and using $\sigma_0 = E_0/\sqrt{2}$ from dimensional analysis:
\begin{equation}
\Delta_{\text{min}} = 2 \cdot \sqrt{2}\sigma_0 \cdot \varphi = 2 \cdot \sqrt{2} \cdot \frac{E_0}{\sqrt{2}} \cdot \varphi = 2E_0 \varphi
\end{equation}
...
For numerical estimates, we use $c_6 \approx 1.4$ based on the required gap value.
