...
\textbf{Step 3: Scaling factor.} We use the golden ratio $\varphi = (1+\sqrt{5})/2$ as the scaling factor. This choice is empirically observed to provide good convergence properties, though other choices $r > 1$ would also work.
...
\begin{equation}
\Delta_{\text{min}} = \sqrt{2} \varphi \cdot E_0 = 0.571 \text{ GeV}
\end{equation}
where $\varphi = (1+\sqrt{5})/2$ is the golden ratio. After accounting for gauge interactions through the dressing factor $c_6 = 2.0 \pm 0.4$ (Appendix~\ref{app:technical}), the physical mass gap is
\begin{equation}
\Delta = c_6 \cdot \Delta_{\text{min}} = 1.13 \pm 0.25 \text{ GeV}
\end{equation}
\end{theorem}
...
where $E_0 = 0.250$ GeV is the fundamental energy scale. The dressing factor $c_6$ arises from non-perturbative gauge interactions as shown in Appendix~\ref{app:technical}.
\end{proof}
...
\begin{equation}
K(n,m) = \frac{1}{(4\pi a)^{3/2}} \exp\left(-\frac{\|S_n - S_m\|_F^2}{4a}\right) > 0
\end{equation}
This is the heat kernel on $\mathfrak{su}(3)$ with the Frobenius metric, which is manifestly positive and acts irreducibly on the gauge sector.
...
&\leq \exp\left(-\frac{\sqrt{2}\varphi E_0}{a} \cdot \text{Area}(R)\right)
\end{align}
where the bound follows from the spectral gap (Lemma~\ref{lem:spectral-gap}): each plaquette contributes at least $\sqrt{2}\varphi E_0/a$ to the cost. This gives the required area law and justifies the Mayer activity bound $|z_b(a)| \leq \exp(-m|b|/a)$ with $m = \sqrt{2}\varphi E_0/2$.
