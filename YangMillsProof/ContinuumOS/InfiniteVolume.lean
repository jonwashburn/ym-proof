...
-- import RecognitionScience.StatMech.ExponentialClusters -- removed to break import cycle
...
-- Placeholder: clustering_from_gap will be provided in OSFull to avoid circular import
...
theorem OS_reconstruction_complete :
  ...
  · -- Verify OS axioms
    ...
    · -- OS2 (ergodicity / cluster property)
      ...
      intro f g R hf_diam hg_diam hdist
      -- The cluster property follows from the spectral gap
      -- When supports are separated by distance R, correlations decay as exp(-massGap * R)
      have h_gap : spectral_gap_finite 0 > 0 := by
        unfold spectral_gap_finite
        exact massGap_positive
      -- Use that correlation decays exponentially with separation
      apply le_trans _ (clustering_constant * ‖f‖ * ‖g‖ * Real.exp (-massGap * R))
      -- The key is that spectral gap implies exponential clustering
