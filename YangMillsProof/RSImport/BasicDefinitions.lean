...
-- Re-export basic constants from Recognition Science
open RecognitionScience

-- Golden ratio and related constants
def phi : ℝ := φ
def E_coh : ℝ := 0.090  -- eV, coherence energy quantum
...
theorem phi_gt_one : 1 < phi := by
  ...
  have h_sqrt5_gt_one : 1 < Real.sqrt 5 := by
    have : 1 < Real.sqrt 5 := Real.one_lt_sqrt_iff.mpr (by norm_num : 1 < 5)
    exact this
  have h_numerator : 2 < 1 + Real.sqrt 5 := by linarith [h_sqrt5_gt_one]
  have h_two_pos : (0 : ℝ) < 2 := by norm_num
  exact (div_lt_iff_lt_mul h_two_pos).mpr (by linarith [h_numerator])
