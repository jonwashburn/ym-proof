...
theorem activity_zero_iff_vacuum (S : LedgerState) :
  ...
  · -- Forward direction: if activity is zero, then S is vacuum
    ...
    let f (n : ℕ) : ℝ := ((S.entries n).debit + (S.entries n).credit) * phi^(n+1)
    have hf : ∀ n, 0 ≤ f n := by
      intro n
      apply mul_nonneg
      · apply add_nonneg
        ...
        · exact (S.entries n).credit_nonneg
      · exact pow_nonneg (le_of_lt phi_pos) (n+1)
    have h_all_zero : ∀ n, f n = 0 := (tsum_eq_zero_iff_all_eq_zero f hf).mp h_zero
    ...
    have h_entries_zero : ∀ n, (S.entries n).debit = 0 ∧ (S.entries n).credit = 0 := by
      ...
      | inl h_sum =>
        ...
        have h_both_zero := le_antisymm (le_of_eq h_sum) h_sum_nonneg
    ...
    apply LedgerState.ext
    intro n
    apply LedgerEntry.ext
  ...
  · -- Backward direction: if S is vacuum, then activity is zero
    ...
    rw [h_vacuum]
    unfold activityCost
    simp only [vacuumState_entries, zero_add, zero_mul, mul_zero]
    exact tsum_zero
