...
def OSPositivityForCorr (R : Reflection) (C : Corr) : Prop :=
  ∀ {ι : Type} [Fintype ι] [DecidableEq ι]
    (f : ι → Observable) (c : ι → Complex),
      0 ≤ (∑ i, ∑ j,
        Complex.conj (c i) * C.eval (f i) (reflect R (f j)) * (c j)).re
...
/-!
Alias spelling requested in the taskboard: correlation-level OS positivity yields
sesquilinear reflection positivity with `S := C.eval`.
-/
theorem RP_sesq_of_corr_OS
    {μ : LatticeMeasure} {R : Reflection} {C : Corr}
    (hHerm : SesqHermitian C.eval)
