...
def RowStochastic : Prop := ∀ i, (∑ j, K.P i j) = (1 : ℝ)
...
def Coercive (ε : ℝ) : Prop := 0 < ε ∧ ∀ i j, ε ≤ K.P i j
...
def Adj (i j : ι) : Prop := 0 < K.P i j

/-- Reachability via some positive-length path (matrix powers). -/
def Reachable (i j : ι) : Prop := ∃ k : Nat, 0 < k ∧ 0 < (K.P ^ k) i j

/-- Strong connectivity phrased as reachability for all pairs. -/
def StronglyConnected : Prop := ∀ i j, K.Reachable i j
...
theorem irreducible_iff_stronglyConnected : K.Irreducible ↔ K.StronglyConnected := Iff.rfl
...
theorem irreducible_of_strictlyPositive (hpos : K.StrictlyPositive) : K.Irreducible := by
  intro i j
